...
class LCAOTDDFTq0(object):
    ...
    def __init__(self,
        ...
        if not isinstance(calc, GPAW):
            calc = GPAW(calc, txt=None,
                        parallel={'kpt': None, 'domain': 1, 'band': 1})
        if calc.wfs.mode is not 'lcao':
    ...
    def get_proj_ani(self, spin=0, k=0):
        ...
        proj_ani = {}
        for nat in range(len(self.calc.wfs.setups.id_a)):
            a_sa = self.calc.wfs.kd.symmetry.a_sa[nsym, nat]
            proj_ani[nat] = dot(kpt.P_ani[a_sa],
                                self.calc.wfs.setups[nat].R_sii[nsym])
        return proj_ani
    ...
    def get_overlap_energydiff(self, k, spin, mynks):
        ...
        for qdir in range(3):
            ...
            gemm(1.0, coeff_nnu, gradcoeff_num, 1.0, overlap_qvnm[qdir], 't')
        ...
        for qdir in range(3):
            ...
            for i in range(0, nbands-1):
                for j in range(i+1, nbands):
                    if omegamin < abs(de_nm[i, j]):
                        if abs(de_nm[i, j]) < omegamax:
