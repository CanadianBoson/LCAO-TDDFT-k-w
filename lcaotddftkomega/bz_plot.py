...
#Before running this code to obtain Brillouin zone plots of the oscillator strengths, make sure the following files are present:
# (1) The GPAW calculation file (.gpw)
# (2) The transitions file: generated by adding the options -t -ct=### to the LCAO-TDDFT-k-w method (.dat)

#Currently, this code supports hexagonal and orthorhombic unit cells only. Note that if a 3D cell is inserted into
...
def bz_arrays(gpaw_file):
    ...
    # (2) An array of mappings from the irreducible BZ (IBZ) to the reducible BZ (RBZ), labeled as 'bz_to_ibz_map.npy'
...
def transition_data(transitionsfile, energy, energy_radius=0.2, time_step=False):
    #This function extracts from the LCAO-TDDFT-k-w generated transitions file the transitions within a certain energy range
    ...
    for i in range(1, len(transitionsdata)):
        ...
        if(float(e_n) > float(energy)-float(energy_radius) and float(e_n) < float(energy)+float(energy_radius)):
...
def relevant_bands(npy_file='oscillator_strengths.npy', matrix_size=20, rounding_factor=1000):
    #This function shows a matrix with the most relevant m --> n (valence --> conduction) band transitions

    #npy_file: the oscillator_strengths.npy file generated in the transition_data() method
    #matrix_size: the width of the matrix (int); an error will occur if transitions occur for band number > matrix_size
    #rounding_factor: this will make the results more presentable, especially if a large transition matrix is used
    ...
    for j in np.arange(0, len(transitions)):
        ...
        for i in index:
            ...
            if(float(eta)==0.0):
                ...
                mat[i][2] += (float(eta)**2/((transitions[j,0]-float(energy))**2 + float(eta)**2))*transitions[j,1]/float(len(index)) 
        if(time_step):
            end = time.time()
...
def brillouinplot_orthorhombic(mat, ax, dashed_outline=True):
    #This function creates a contour plot for an orthorhombic unit cell. python3 is required for these contours to be generated
...
def brillouinplot_hexagonal(mat, ax, dashed_outline=True, mask=True):
    #This function creates a contour plot for a hexagonal unit cell. python3 is required for these contours to be generated

    #mat: the weighted_transitions file (.npy)
    #ax: plot (subplot) environment
    #dashed_outline: create a dashed outline around the BZ plot
    #mask: mask all transitions beyond the contours; set to False for cool looking plots

    #This utilizes the 6-fold symmetry of the graphene BZ to superimpose data rotated 120 and 240 degrees
    npts = len(mat)
    ngridx = 800
    ngridy = 800
    ...
    x=[0.3333+1.0/2.0*0.3333 , 0.0, -0.3333-1.0/2.0*0.3333, -0.3333-1.0/2.0*0.3333, 0.0, 0.3333+1.0/2.0*0.3333]
    y=[np.sqrt(3)/2.0*0.3333, 1.0/np.sqrt(3), np.sqrt(3)/2.0*0.3333, - np.sqrt(3)/2.0*0.3333, -1.0/np.sqrt(3), - np.sqrt(3)/2.0*0.3333]
...
def mask_outside_polygon(poly_verts, ax=None): 
    ...
    #Credit goes to: https://stackoverflow.com/questions/3320311/fill-outside-of-polygon-mask-array-where-indicies-are-beyond-a-circular-bounda
