...
"""Test for domain parallelization 
and ScaLAPACK implementation within
...
def main():
    ...
    calc_sl = GPAW(name, parallel={'sl_auto': True})
    lcao_tddft_sl = LCAOTDDFTq0(calc_sl, eta=0.2, verbose=True)
    lcao_tddft_sl.use_hilbert_transform(True)
    grad_phi_sl = lcao_tddft_sl.grad_phi_kqvnumu
    equal(max(abs(grad_phi_sl - grad_phi)), 0, 1e-8)
    epsilon = lcao_tddft_sl.get_epsilon()
    lcao_tddft_sl.write_dielectric_function(name.split('.gpw')[0])
    if world.rank == 0:
        energies = epsilon[0]
