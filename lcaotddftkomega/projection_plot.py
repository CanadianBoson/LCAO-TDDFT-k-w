...
def projection_data_y(cube_file, cube_projection, noproj = False, voxel = '0'):
    ...
    spacing_vec, values = read_cube_data(cube_file)
    idx=-1
    length = int(spacing_vec[1][0])*float(spacing_vec[1][2])*0.5291772083
    # This yields the length of unit cell in Angstrom
...
def projection_data_x(cube_file, cube_projection, noproj = False, voxel = '0'):
    ...
    spacing_vec, values = read_cube_data(cube_file)
    idx=-1
    length = int(spacing_vec[0][0])*float(spacing_vec[0][1])*0.5291772083
    # This yields the length of unit cell in Angstrom
    voxel = voxel
    voxel = float(voxel)
...
def contour_maker(dtype = 'e', direction = 'z', cubedir='.', coarseness=1, brightness=0.75, emin=0.2, emax=20, ax=p, xenergy=False, noproj=False, voxel = '0'):
    ...
    # direction: 'x', 'y', or 'z', will project all cube directions onto the chosen axis
